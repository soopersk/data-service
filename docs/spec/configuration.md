...
| Property | Default | Description |
|----------|---------|-------------|
| `observability.sla.duration-based.enabled` | `true` | DURATION mode only: derive the SLA deadline from the resolved baseline; `false` leaves runs ungraded |
| `observability.sla.duration-based.threshold-percent` | `20` | DURATION mode only: percentage buffer over the resolved baseline |
...
| `observability.sla.lookback.daily-days` | `30` | Shared (both modes): trailing window for DAILY baselines/profiles |
| `observability.sla.lookback.monthly-days` | `395` | Shared (both modes): trailing window for MONTHLY baselines/profiles |
